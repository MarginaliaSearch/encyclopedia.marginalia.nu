...
public class ZIMReader {
	...
	public DirectoryEntry forEachArticles(BiConsumer<String, String> consumer, Predicate<Integer> blobPred)
		...
		System.out.println("Indexing ZIM file");

		for (long i = beg; i < end; i+=4) {
			var entry = getDirectoryInfoAtTitlePosition(i);

			if (((i-beg)%100_000) == 0) {
				System.out.printf("\u001b[2K\r%3.2f%%", ((i-beg) * 100.) / (end-beg));
			}
		...
		System.out.println("Iterating over " + data.keySet().stream().mapToInt(Integer::intValue).max().orElse(0) + "clusters");
